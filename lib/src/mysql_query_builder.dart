...
import 'builders/value.dart' as mysql;
import 'row_instantiator.dart' as mysql;
import 'builders/sort.dart' as mysql;

class MySqlQueryBuilder extends mysql.TableBuilder {
  MySqlQueryBuilder(MySqlQuery query) : super(query) {
  ...
  final List<mysql.ColumnValueBuilder> columnValueBuilders = [];

  bool get containsJoins =>
      returning.reversed.any((p) => p is mysql.TableBuilder);
  ...
  List<T> instancesForRows<T extends ManagedObject>(List<Row> rows) {
    final instantiator = mysql.RowInstantiator(this, returning);
    return instantiator.instancesForRows<T>(rows);
  }

  mysql.ColumnValueBuilder _createColumnValueBuilder(
    ...
    if (property is ManagedRelationshipDescription) {
      ...
      if (value != null) {
        if (value is ManagedObject || value is Map) {
          return mysql.ColumnValueBuilder(
    ...
    return mysql.ColumnValueBuilder(this, property, value);
  }
  ...
  String get sqlOrderBy {
    var allSorts = List<mysql.ColumnSortBuilder>.from(columnSortBuilders);

    var nestedSorts = returning
        .whereType<mysql.TableBuilder>()
