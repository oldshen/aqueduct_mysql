...
import 'sort.dart' as mysql;
import 'expression.dart' as mysql;

class TableBuilder implements mysql.Returnable {
  TableBuilder(MySqlQuery query, {this.parent, this.joinedBy})
      : entity = query.entity,
    ...
    returning =
        mysql.ColumnBuilder.fromKeys(this, query.propertiesToFetch ?? []);

    columnSortBuilders = query.sortDescriptors
            ?.map((s) => mysql.ColumnSortBuilder(this, s.key, s.order))
            ?.toList() ??
        [];

    if (query.pageDescriptor != null) {
      columnSortBuilders.add(mysql.ColumnSortBuilder(
      ...
      if (query.pageDescriptor.boundingValue != null) {
        ...
        final expr = mysql.ColumnExpressionBuilder(this, prop,
    ...
    query.subQueries?.forEach((relationshipDesc, subQuery) {
      addJoinTableBuilder(TableBuilder(subQuery as MySqlQuery,
  ...
  final List<mysql.ColumnExpressionBuilder> expressionBuilders = [];
  String tableAlias;
  QueryPredicate predicate;
  List<mysql.ColumnSortBuilder> columnSortBuilders;
  List<mysql.Returnable> returning;
  int aliasCounter = 0;
  ...
  List<mysql.ColumnBuilder> get flattenedColumnsToReturn {
    return returning.fold(<mysql.ColumnBuilder>[], (prev, c) {
      if (c is TableBuilder) {
        prev.addAll(c.flattenedColumnsToReturn);
      } else if (c is mysql.ColumnBuilder) {
        prev.add(c);
      }
      return prev;
    });
  ...
  QueryPredicate get joiningPredicate {
    mysql.ColumnBuilder left, right;
    if (identical(foreignKeyProperty, joinedBy)) {
      left = mysql.ColumnBuilder(parent, joinedBy);
      right = mysql.ColumnBuilder(this, entity.primaryKeyAttribute);
    } else {
      left = mysql.ColumnBuilder(parent, parent.entity.primaryKeyAttribute);
      right = mysql.ColumnBuilder(this, joinedBy.inverse);
    }
    ...
    expressions.forEach((expression) {
      ...
      if (isPropertyOnThisEntity) {
        ...
        if (isColumn) {
          ...
          final expr = mysql.ColumnExpressionBuilder(
