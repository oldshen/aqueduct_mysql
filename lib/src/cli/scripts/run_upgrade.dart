...
class RunMySqlUpgradeExecutable extends Executable<Map<String, dynamic>> {
  RunMySqlUpgradeExecutable(Map<String, dynamic> message)
  ...
  Future<Map<String, dynamic>> execute() async {
    ...
    MySqlPersistentStore.logger.level = Level.ALL;
    MySqlPersistentStore.logger.onRecord
        .listen((r) => log("${r.message}"));

    PersistentStore store;
    if (dbInfo != null && dbInfo.flavor == "mysql") {
      store = MySqlPersistentStore(dbInfo.username, dbInfo.password,
    ...
    try {
      ...
      if (e.errorNumber ==MySqlErrorCode.invalid_use_of_NULL_value.code &&
          e.message.contains("Invalid use of NULL value")) {
        return {
          "error":
              //"There was an issue when adding or altering column '${e.tableName}.${e.columnName}'. "
              "There was an issue when adding or altering column '${e.message}'. "
      ...
      return {
        "error":
            // "There was an issue. Reason: ${e.message}. Table: ${e.tableName} Column: ${e.columnName}"
             "There was an issue. Reason: ${e.message}. "
      };
    }
  }
  ...
  static List<String> get imports => [
        ...
        "package:aqueduct_mysql/mysql.dart",
